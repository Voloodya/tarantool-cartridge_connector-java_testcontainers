...
local function hello()
    return "Hello from Tarantool cluster cartridge!"
end
...
local function get_schema()
    for _, instance_uri in pairs(cartridge_rpc.get_candidates('app.roles.router', { leader_only = true })) do
        return cartridge_rpc.call('app.roles.router', 'get_schema', nil, { uri = instance_uri })
    end
end
...
local function init(opts) -- luacheck: no unused args
    ...
    --- Выставление наружу хранимой процедуры. _G - глобальная таблица lua, хранящая метаданные lua-интерпритатора
    rawset(_G,"hello", hello)
